...
### 成果
- [x] 审查并优化现有 CI workflow
- [x] 集成能力资产质量检查
- [x] 集成索引同步验证
- [x] 添加 TODO/FIXME 扫描
- [x] 优化执行速度和错误提示
- [x] 移除损坏的 duty-check workflow

### 输出
1. ✅ 增强的 .github/workflows/check.yml
   - 结构验证
   - 质量门禁（100% 标准）
   - 索引同步检查
   - TODO 扫描
...
1. **自动化质量保障**
   - check-capabilities.sh 集成
   - 100% 质量标准强制执行

2. **索引一致性**
   - sync-capability-index.sh 验证
   - 防止手动更新遗漏
...
2. **scheduled workflows 的局限**
   - GitHub Actions 定时任务不适合巡检
   - 本地 cron 或手动触发更可靠
